            ...
            <div className={`${!DropDownOpen && 'hidden'}  absolute top-6  font-normal bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600 `}>
                <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownLargeButton">
                    {data.links.map((el, index) => (
                        <li key={index}>
                            <Link href={el.path} className="block px-4 py-2 font-bold text-red-900 hover:underline bg-transparent transition duration-300 ease-in-out transform hover:scale-110">{el.title}</Link >
                        </li>
                    ))}
